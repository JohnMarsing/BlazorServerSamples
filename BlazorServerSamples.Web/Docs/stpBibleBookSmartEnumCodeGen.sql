...
/*
  ...
  Note
	...
	INSERT INTO @BibleBookTable (Id, Value, Title, Abrv, BookGroupEnum, BookEnum, LastChapter, TransliterationInHebrew, NameInHebrew )
	SELECT Id, CAST(Id AS varchar(10)), REPLACE([Title], ' ', '')
	, Abrv, CAST(BookGroupId AS varchar(10)) AS BookGroupEnum, CAST(Id AS varchar(10)) AS BookEnum, CAST(LastChapter AS varchar(10)) AS LastChapter
	, HebrewTitle AS TransliterationInHebrew, HebrewName AS NameInHebrew
	FROM Bible.Book 

	+ @CRLF + '	public ' + [Title] + @InstanceSuffix + '() : base($"{nameof(Id.' + [Title] + ')}", Id.' + [Title] + ') { }' 
...
', ' + QUOTENAME([Month], CHAR(34)) ', ' + QUOTENAME([Month], CHAR(39)) +

	-- This Doesn't work
	-- Manually delete the rows not used and replace **TYPE** with approriate csharp types
	SET @ExtraFields = 'Abrv,BookGroupEnum,BookEnum,LastChapter,TransliterationInHebrew,NameInHebrew'
	SELECT 
	'public abstract ' + dbo.udfCsharpTypeLookup(DATA_TYPE) + ' ' + COLUMN_NAME + ' { get; }'  AS CodeGenExtraFields
	FROM INFORMATION_SCHEMA.COLUMNS 
	INNER JOIN	STRING_SPLIT ( @ExtraFields , ',' ) ST_SPLIT  ON COLUMN_NAME=ST_SPLIT.value
	WHERE TABLE_NAME=@Table


Not needed because it uses BookEnum and BookGroupEnum
	--SELECT 'private static class Id ' + @CRLF + '{' AS Head
	SELECT @TAB + 'internal const int ' + [Title] + ' = ' + Value +  + ';' + @CRLF AS CodeGenIds
	FROM  @BibleBookTable ORDER BY Id
	--SELECT '}' AS Foot
